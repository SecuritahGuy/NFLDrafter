          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <section>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                {injuries.length ? <div className="space-y-3">{injuries.slice(0, 3).map((injury, index) => <div key={`${injury.source}-${injury.season}-${injury.week}-${index}`} className="border-b border-slate-100 pb-3 last:border-0 last:pb-0"><div className="flex items-center justify-between gap-3"><span className="font-bold text-slate-900">{injury.primary_injury ?? 'Practice report'}</span><span className="text-xs font-bold text-rose-700">{injury.report_status ?? injury.practice_status ?? 'Listed'}</span></div><div className="text-xs text-slate-500">{injury.source} · {injury.is_current ? 'Current' : `${injury.season} · Week ${injury.week}`}</div></div>)}</div> : <p className="text-sm text-slate-600">No current injury-report entries.</p>}
              </div>
            </section>
