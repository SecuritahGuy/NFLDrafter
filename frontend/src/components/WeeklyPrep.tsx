...
import { ArrowPathIcon, ChartBarIcon, ExclamationTriangleIcon, UserGroupIcon } from '@heroicons/react/24/outline'
...
type YahooRoster = {
  team_id: string
  players: Array<{ id: string; canonical_player_id?: string; name: string; position: string; team: string; selected_position?: string }>
}
...
type YahooSnapshot = {
  ...
  mapping_coverage?: { season: number; mapped_roster_players: number; rostered_players: number }
  teams: YahooTeam[]
  rosters: YahooRoster[]
    ...
    <div className="min-h-[calc(100vh-3rem)] bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-6">
        ...
        <section className="mt-6 grid gap-3 sm:grid-cols-3" aria-label="Data source health">
          ...
          <SourceHealth label="Official injury reports" value={String(injuryByPlayerId.size)} detail="Current season reports available" tone="neutral" />
        </section>
        ...
        <section className="mt-8 grid gap-4 lg:grid-cols-2">
          <div className="rounded-xl border border-slate-700 bg-slate-900/70 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><UserGroupIcon className="h-5 w-5 text-emerald-300" /><h2 className="font-black">Roster inspector</h2></div><select aria-label="Inspect team roster" value={inspectedTeam?.id || ''} onChange={event => setInspectedTeamId(event.target.value)} className="rounded-lg border border-slate-600 bg-slate-800 px-2 py-1.5 text-sm font-semibold text-slate-100">{snapshot.teams.map(team => <option key={team.id} value={team.id}>{team.name}{team.is_current_user ? ' (You)' : ''}</option>)}</select></div><p className="mt-1 text-sm text-slate-400">{inspectedTeam?.is_current_user ? 'Your roster and assigned lineup slots.' : `${inspectedTeam?.name || 'Selected team'} roster.`}</p><div className="mt-4 flex flex-wrap gap-2">{inspectedRoster.length ? inspectedRoster.map(player => <span key={player.id} className="rounded-lg border border-slate-700 bg-slate-800 px-2.5 py-1.5 text-xs"><span className="font-bold">{player.name}</span> <span className="text-slate-400">{player.position} · {player.selected_position || '—'}</span></span>) : <p className="text-sm text-slate-400">No roster data was included in this snapshot.</p>}</div></div>
