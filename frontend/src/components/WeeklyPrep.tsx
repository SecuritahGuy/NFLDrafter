...
const plausibleProjection = (position: string | null, points: number | null, ppg: number | null) => {
  ...
  const ppgCeiling = ['K', 'DST'].includes(normalized || '') ? 20 : 40
  const seasonCeiling = ['K', 'DST'].includes(normalized || '') ? 300 : 700
  return ppg <= ppgCeiling && points <= seasonCeiling
}
...
export const WeeklyPrep: React.FC = () => {
  ...
  const { data: fantasyProsProjections } = useRankings('fantasypros-projection')
  const { data: espnProjections } = useRankings('espn-draft-rank')
  ...
  const projectionByPlayerId = useMemo(() => {
    const blended = new Map<string, ProjectionSignal>()
    for (const row of fantasyProsProjections?.rankings || []) {
      if (row.player_id && plausibleProjection(row.position, row.projected_points, row.projected_points_per_game)) blended.set(row.player_id, { projected_points: row.projected_points, projected_points_per_game: row.projected_points_per_game, source: 'FantasyPros' })
    }
    for (const row of espnProjections?.rankings || []) {
      if (row.player_id && !blended.has(row.player_id) && plausibleProjection(row.position, row.projected_points, row.projected_points_per_game)) blended.set(row.player_id, { projected_points: row.projected_points, projected_points_per_game: row.projected_points_per_game, source: 'ESPN' })
    }
    return blended
  ...
  const waiverTargets = (snapshot.players || []).flatMap(player => {
    ...
    return [{ player, projectedPpg: projection.projected_points_per_game, projectionSource: projection.source }]
    ...
    <div className="min-h-[calc(100vh-3rem)] bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-6xl px-5 py-8 sm:px-6">
        ...
        <section className="mt-6 grid gap-3 sm:grid-cols-3" aria-label="Data source health">
          ...
          <SourceHealth label="Projection coverage" value={`${[...teamProjection.values()].reduce((total, item) => total + item.covered, 0)}/${snapshot.mapping_coverage?.mapped_roster_players ?? 0}`} detail={`FantasyPros ${fantasyProsProjections?.snapshot_date || '—'} · ESPN ${espnProjections?.snapshot_date || '—'}`} tone={projectionByPlayerId.size ? 'good' : 'warning'} />
        ...
        <section className="mt-8 rounded-2xl border border-emerald-400/20 bg-emerald-950/20 p-6">
          ...
          <div className="mt-5 grid gap-3 lg:grid-cols-2">
            ...
            {lineupAdvice.benchUpside.map(({ signal, starter }) => <AdviceCard key={`upside-${signal.player.id}`} tone="good" title={`Compare ${signal.player.name} with ${starter.player.name}`} detail={`${signal.player.position} bench · ${signal.projectedPpg?.toFixed(1)} ${signal.projectionSource || 'projection'} PPG vs ${starter.projectedPpg?.toFixed(1)} ${starter.projectionSource || 'projection'} PPG for the current starter · season projection only`} />)}
        ...
        <section className="mt-8 rounded-2xl border border-violet-400/20 bg-violet-950/20 p-6">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">This is a read-only shortlist from the available Yahoo player pool, excluding everyone on a cached league roster, ordered by cached FantasyPros projections with ESPN fallback. It is not an automatic add recommendation; compare fit, injury risk, and current-week context before making a move.</p>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{waiverTargets.map(({ player, projectedPpg, projectionSource }) => <div key={player.id} className="rounded-xl border border-violet-400/20 bg-slate-950/40 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-violet-100">{player.name}</p><p className="mt-1 text-xs text-slate-400">{player.position} · {player.team || 'FA'}</p></div><span className="text-right text-sm font-black text-violet-200">{projectedPpg.toFixed(1)}<span className="ml-1 text-[10px] font-bold uppercase text-slate-400">PPG</span></span></div><div className="mt-3 flex items-center justify-between gap-2 text-xs text-slate-400"><span>Yahoo rostered: {typeof player.percent_owned === 'number' ? `${player.percent_owned.toFixed(0)}%` : 'Not available'}</span><span className="rounded bg-violet-400/10 px-1.5 py-0.5 font-bold text-violet-200">{projectionSource}</span></div></div>)}{!waiverTargets.length ? <p className="rounded-xl border border-slate-700 bg-slate-900/60 p-4 text-sm text-slate-300">No available players currently have both a verified identity and a cached projection. Refresh sources to expand the local coverage.</p> : null}</div>
        </section>
        ...
        <section className="mt-8">
          ...
          <p className="mt-1 text-sm text-slate-400">Standings, roster depth, and cached FantasyPros projections with ESPN fallback at a glance.</p>
      ...
      {drawerTeamId && drawerTeam ? <TeamRosterDrawer team={drawerTeam} starters={drawerStarters} bench={drawerBench} projectionSummary={teamProjection.get(drawerTeamId)} isLoading={isLoadingTeam} error={teamDetailError} projectionDates={{ fantasyPros: fantasyProsProjections?.snapshot_date || null, espn: espnProjections?.snapshot_date || null }} statsSeasons={teamDetail?.stats_seasons || []} onClose={() => setDrawerTeamId(null)} /> : null}
    </div>
  )
}
...
const TeamRosterDrawer: React.FC<{
  ...
  statsSeasons: number[]
  onClose: () => void
}> = ({ team, starters, bench, projectionSummary, isLoading, error, projectionDates, statsSeasons, onClose }) => {
