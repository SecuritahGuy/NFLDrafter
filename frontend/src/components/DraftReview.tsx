...
export const DraftReview: React.FC = () => {
  ...
  return <div className="min-h-[calc(100vh-3rem)] bg-slate-950 text-slate-100">
    <main className="mx-auto max-w-6xl px-5 py-8 sm:px-6">
      ...
      <div className="mt-6 rounded-xl border border-amber-400/25 bg-amber-400/10 p-4 text-sm leading-6 text-amber-100"><strong>Snapshot note:</strong> this first report uses the cached player board available in this browser. Historical rank snapshots are preserved by source when available, but automatic nearest-draft-time selection is the next review enhancement.</div>
      <section className="mt-6 space-y-4">
        {myPicks.map(pick => {
          const chosen = playersById.get(pick.playerId)
          const alternatives = session.picks.filter(candidate => candidate.pick > pick.pick).map(candidate => ({ pick: candidate, player: playersById.get(candidate.playerId) })).filter((candidate): candidate is { pick: DraftPick; player: Player } => Boolean(candidate.player)).sort((a, b) => playerOrder(a.player) - playerOrder(b.player)).slice(0, 3)
          return <article key={pick.pick} className="rounded-2xl border border-slate-700 bg-slate-900/70 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-wide text-slate-500">Pick {pickLabel(pick, session.config.leagueSize)} · overall #{pick.pick}</p><h2 className="mt-1 text-xl font-black">{chosen?.name ?? pick.playerId}</h2><p className="mt-1 text-sm text-slate-400">{chosen ? `${chosen.position} · ${chosen.team || 'FA'}${chosen.rank ? ` · cached composite #${chosen.rank}` : ''}` : 'Player identity was not retained in the local package.'}</p></div><span className="rounded-lg bg-emerald-400/10 px-3 py-1.5 text-xs font-bold text-emerald-200">Your selection</span></div><div className="mt-4 border-t border-slate-700 pt-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">Best cached board alternatives selected later</p><div className="mt-3 grid gap-2 md:grid-cols-3">{alternatives.map(({ pick: alternativePick, player }) => <div key={alternativePick.pick} className="rounded-xl border border-slate-700 bg-slate-950/50 p-3"><p className="font-bold">{player.name}</p><p className="mt-1 text-xs text-slate-400">{player.position} · {player.team || 'FA'} · selected {pickLabel(alternativePick, session.config.leagueSize)}</p><p className="mt-2 text-xs font-semibold text-cyan-200">{player.rank ? `Cached composite #${player.rank}` : player.adp ? `Cached ADP ${player.adp.toFixed(1)}` : 'No cached ordering'}</p></div>)}{!alternatives.length && <p className="text-sm text-slate-400">No later selections with a cached player record.</p>}</div></div></article>
        })}
      </section>
